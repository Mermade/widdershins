...
| --resolve | options.resolve | boolean | false | Resolve external $refs, using the `source` parameter as the base location |
| --shallowSchemas | options.shallowSchemas | boolean | false | Don't expand schemas past $refs |
| --source | options.source | string | | The absolute location or URL of the source file to resolve $refs from; required if `resolve` is set to true |
