	...
	for (var r in apiInfo.resources) {
		...
		for (var m in resource.methods) {
			...
			if ((method.op != 'parameters') && (!method.op.startsWith('x-'))) {
				...
				var sharedParameters = swagger.paths[method.path].parameters || [];
				var opParameters = (swagger.paths[method.path][method.op].parameters || []);
				...
				for (var p in sharedParameters) {
					if (sharedParameters[p]["$ref"]) {
						sharedParameters[p] = jptr.jptr(swagger, sharedParameters[p]["$ref"]);
					}
				}
				for (var p in opParameters) {
					if (opParameters[p]["$ref"]) {
						opParameters[p] = jptr.jptr(swagger, opParameters[p]["$ref"]);
					}
				}
