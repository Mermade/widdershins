...
/**
* function to reformat asyncapi paths object into an iodocs-style resources object, tags-first
*/
...
function convert(asyncapi, options, callback) {
	...
	if (typeof templates === 'undefined') {
		templates = dot.process({ path: path.join(__dirname, 'asyncapi', 'templates') });
	}
	...
	for (var r in apiInfo.resources) {
		...
		for (var m in resource.methods) {
			...
			var op = asyncapi.paths[method.path][method.op];
			if ((method.op != 'parameters') && (!method.op.startsWith('x-'))) {
				...
				if (op.requestBody) {
					if (op.requestBody.$ref) {
						op.requestBody = jptr.jptr(asyncapi,op.requestBody.$ref);
					}
				...
				for (var r in op.responses) {
					...
					if (response.$ref) {
						response = jptr.jptr(asyncapi,response.$ref);
					}
				...
				data.operationId = asyncapi.paths[method.path][method.op].operationId;
				data.tags = asyncapi.paths[method.path][method.op].tags;
				data.security = asyncapi.paths[method.path][method.op].security;
				...
				var sharedParameters = asyncapi.paths[method.path].parameters || [];
				var opParameters = (asyncapi.paths[method.path][method.op].parameters || []);
				...
				for (var p in sharedParameters) {
					if (sharedParameters[p]["$ref"]) {
						sharedParameters[p] = jptr.jptr(asyncapi, sharedParameters[p]["$ref"]);
					}
				}
				for (var p in opParameters) {
					if (opParameters[p]["$ref"]) {
						opParameters[p] = jptr.jptr(asyncapi, opParameters[p]["$ref"]);
					}
				}
				...
				for (var p in parameters) {
					...
					param.exampleSchema = parameterToSchema(param, asyncapi);
					param.exampleValues = {};
				...
				if (parameters.length > 0) {
					...
					if (longDescs) {
						for (var p in parameters) {
							var param = parameters[p];
							//if (param["$ref"]) {
							//    param = jptr.jptr(asyncapi,param["$ref"]);
							//}
					...
					for (var p in parameters) {
						...
						if (param.schema) {
							...
							var xmlWrap = '';
							var obj = dereference(param.schema, asyncapi);
				...
				if (responseSchemas) {
					...
					for (var resp in op.responses) {
						...
						if (response.schema) {
							var xmlWrap = '';
							var obj = dereference(response.schema, asyncapi);
							if (obj.xml && obj.xml.name) {
								xmlWrap = obj.xml.name;
							}
				...
				var security = (op.security ? op.security : asyncapi.security);
