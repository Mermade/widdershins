...
function convertToToc(source) {
    ...
    for (let c in source.channels) {
        ...
        for (var i in channel) {
            ...
            resources[tagName].channels[c] = { items: {}, parameters: channel.parameters };
            resources[tagName].channels[c].items[i] = item;
        }
    }
    return resources;
}
...
function convertInner(api, options, callback) {
    return new Promise(function(resolve, reject){
        ...
        let header = {};
        header.title = ((api.info && api.info.title) ? api.info.title : 'API') + ' ' + data.version || '1.0.0';
