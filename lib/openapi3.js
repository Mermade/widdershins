    ...
    for (let r in data.operation.responses) {
        ...
        for (let ct in response.content) {
            ...
            if (contentType.schema && contentType.schema["x-widdershins-oldRef"] && contentType.schema["x-widdershins-oldRef"].startsWith('#/components/')) {
                ...
                entry.schema = '['+schemaName+'](/reference#'+schemaName.toLowerCase()+')';
                entry.$ref = true;
            }
            else {
...
function createSplitContent(templates, header, data, options, callback) {
    ...
    switch(sectionName) {
        case "customers":
            resources = createContent(`customer-resources`, templates, header, data, options, callback, options.omitHeader);
            reference = createContent(`customer-reference`, templates, header, data, options, callback, true);
            break;
        case "hackers":
            resources = createContent(`hacker-resources`, templates, header, data, options, callback, options.omitHeader);
            reference = createContent(`hacker-reference`, templates, header, data, options, callback, true);
            break;
        default:
    }
